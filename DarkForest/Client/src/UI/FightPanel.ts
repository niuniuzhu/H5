...
export class FightPanel implements IMainPanel {
	...
	public Enter(): void {
		...
		this.CreateOpponent();
		this.PerformFight();
		this._root.getChild("atk0").text = CUser.atk.toString();
		this._root.getChild("def0").text = CUser.def.toString();
	...
	private CreateOpponent(): void {
		this._opAtk = RC.Numerics.MathUtils.Round(RC.Numerics.MathUtils.Max(Math.random() * 20 + 10, CUser.atk + (Math.random() * 40 - 20)));
		this._opDef = RC.Numerics.MathUtils.Round(RC.Numerics.MathUtils.Max(Math.random() * 20 + 10, CUser.def + (Math.random() * 40 - 20)));
	}
	...
	private PerformFight(): void {
		...
		for (let i = 0; i < FightPanel.MAX_ROUND; ++i) {
			let hurt0 = RC.Numerics.MathUtils.Max(0, CUser.atk - this._opDef);
			hurt0 += Math.random() * hurt0 * 0.4 - hurt0 * 0.2;
			let hurt1 = RC.Numerics.MathUtils.Max(0, this._opAtk - CUser.def);
