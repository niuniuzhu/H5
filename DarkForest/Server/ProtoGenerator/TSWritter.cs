...
namespace ProtoGenerator
{
	public class TSWritter : IWriter
	{
		public bool WriteDesc( string ns, Dictionary<string, int> clsToMsgID, Dictionary<string, string> responseToMsgID, string outputPath, ref string error )
		{
			...
			sb.AppendLine( "import protobuf = require( \"protobufjs\" );" );
			sb.AppendLine();
			...
			sb.AppendLine( $"\tpublic static DecodeMsg( msgID:{ns}.MsgID, data:Uint8Array, offset:number, size:number ):any {{" );
			sb.AppendLine( "\t\tswitch ( msgID ) {" );
			foreach ( KeyValuePair<string, int> kv in clsToMsgID )
			{
				sb.AppendLine( $"\t\t\tcase {kv.Value}: {{" );
				sb.AppendLine( "\t\t\t\tlet reader = new protobuf.Reader( data );" );
				sb.AppendLine( "\t\t\t\treader.pos = offset;" );
				sb.AppendLine( "\t\t\t\treader.len = size; " );
				sb.AppendLine( $"\t\t\t\tlet msg = {ns}.{kv.Key}.decode( reader );" );
				sb.AppendLine( "\t\t\t\treturn msg;" );
				sb.AppendLine( "\t\t\t}" );
			}
			sb.AppendLine( "\t\t}" );
			...
			foreach ( KeyValuePair<string, int> kv in clsToMsgID )
			{
				sb.AppendLine( $"\tpublic static D_{kv.Key}( data:Uint8Array, offset:number, size:number ):{ns}.{kv.Key} {{" );
				sb.AppendLine( "\t\tlet reader = new protobuf.Reader( data );" );
				sb.AppendLine( "\t\treader.pos = offset;" );
				sb.AppendLine( "\t\treader.len = size; " );
				sb.AppendLine( $"\t\tlet msg = {ns}.{kv.Key}.decode( reader );" );
			...
			sb.AppendLine( "\t\tlet msgID = message.GetMsgID();" );
			sb.AppendLine( "\t\tswitch ( msgID ) {" );
			foreach ( KeyValuePair<string, int> kv in clsToMsgID )
			{
				sb.AppendLine( $"\t\t\tcase {kv.Value}: {{" );
